...
function drawChart(data) {
  ...
  const y01z = d3
    .stack()
    .keys(sectors) // Stack by sectors
    (data) // Use the original filtered data
    .map((data, i) => data.map(([y0, y1]) => [y0, y1, i]));
  ...
  const x = d3
    .scaleBand()
    .domain(years) // Map index to years
  ...
  const rect = svg
    ...
    .join("rect")
    .attr("x", (d, i) => x(years[i])) // Position based on year index
  ...
  svg
    ...
    .call(
      d3.axisBottom(x)
        ...
        .tickFormat((x) => years[x]) // Label with corresponding year
        .tickSizeOuter(0)
    );
  ...
  function transitionGrouped() {
    ...
    rect
      ...
      .duration(500)
      .delay((d, i) => i * 20)
      .attr("x", (d, i) => x(years[i]) + (x.bandwidth() / m) * d[2]) // Adjust X for group positioning
      .attr("width", x.bandwidth() / m) // Adjust width for grouping
      .transition()
  ...
  function transitionStacked() {
    ...
    rect
      ...
      .transition()
      .attr("x", (d, i) => x(years[i])) // Reset X for stacking
